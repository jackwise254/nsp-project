...
class Nurses(models.Model):
    ...
    # available_days = models.ForeignKey(Days, on_delete=models.CASCADE, null=True)
    available_days = models.ManyToManyField(Days)

    shift_type = models.CharField(max_length=200, choices=SHIFT_TYPE_CHOICES, default=Days)
    ...
    def clean(self):
        ...
        weekday_count = self.available_days.filter(day__in=[1, 2, 3, 4]).count()
        weekend_count = self.available_days.filter(day__in=[5, 6]).count()

        if weekday_count > 14:
            raise ValidationError('Weekday can only be selected 14 times.')

        if weekend_count > 16:
            raise ValidationError('Weekend can only be selected 16 times.')
